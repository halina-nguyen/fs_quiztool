# Driverless Cup Acceleration Scoring
# Source: FSG Rules 2024, D 5.6

import tkinter as tk
from tkinter import ttk
from src.common.penalties import acceleration_doo_penalty as doo
from src.common.p_max import p_max_acceleration_dc as p_max

class AccelerationDCTab:
    def __init__(self, notebook):
        self.tab = ttk.Frame(notebook)
        self.setup_ui()

    # at least one run without OC, USS, DNF or DQ
    def calculate_acceleration_dc(self):
        score = 0
        ...
        t_max *= 2 + (t_max_cones * doo)

        if t_team < t_max:
            score = 0.95 * p_max * ((t_max/t_team) - 1)
        score += 0.05 * p_max

        self.score_acceleration_dc.config(text=f"Acceleration DC score: {score:.2f}")
    ...
    def setup_ui(self):
        ...
        calculate_button = tk.Button(self.tab, text="Calculate Acceleration DC Score", command=self.calculate_acceleration_dc)
        calculate_button.grid(row=5, column=0, columnspan=10, pady=10)

        self.score_acceleration_dc = tk.Label(self.tab, text="")
        self.score_acceleration_dc.grid(row=6, column=0, columnspan=10, pady=10)