# Manual Acceleration Scoring
# Source: FSG Rules 2024, D 5.4

import tkinter as tk
from tkinter import ttk
from src.common.penalties import acceleration_doo_penalty as doo
from src.common.p_max import p_max_acceleration as p_max

class AccelerationTab:
    ...
    def calculate_acceleration(self):
        ...
        if t_team < t_max:
            score = 0.95 * p_max * (((t_max/t_team) - 1) / 0.5)
        score += 0.05 * p_max

        self.score_acceleration.config(text=f"Acceleration score: {score:.2f}")
    ...
    def setup_ui(self):
        ...
        calculate_button = tk.Button(self.tab, text="Calculate Acceleration Score", command=self.calculate_acceleration)
        calculate_button.grid(row=5, column=0, columnspan=10, pady=10)

        self.score_acceleration = tk.Label(self.tab, text="")
        self.score_acceleration.grid(row=6, column=0, columnspan=10, pady=10)